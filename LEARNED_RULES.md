...
21. [DATA] Never accumulate Kalshi `*_fp` quantity strings as raw floats — snap every accumulation to the 2-decimal grid (`round(q, 2)`). A fully consumed level can leave +1e-13 residue that survives `new_qty <= 0` and becomes an immortal phantom level pinning best-ask at a stale price. Verified live 2026-07-05: scanner quoted CLE YES 0.29 vs real 0.36 for 76+s, SFCOL-COL 0.77 vs real 0.89 — the false-arb class the user reported.

22. [PROCESS] Always validate plan test fixtures against fees.py arithmetic and the F1–F4 threshold interplay before implementing them — 4 of the ML-ghost plan's fixture sets were mathematically unreachable as written (synthetic Poly books crossed via yes_bid = 1 − opp_ask; a "confirms and opens" total of 0.96315 ≥ ARB_THRESHOLD; a two-way edge-cap case that F2 caps below 10¢). Caught 2026-07-05 during subagent-driven execution; corrections in DECISIONS.md.

23. [DATA] Never join matcher output to venue caches by exact team-name strings — matcher `kalshi_team`/`polymarket_team` live in per-sport normalized namespaces (tennis `normalize_name`, basketball `team_code`) while the Poly cache holds raw abbrs/participants, so exact joins silently miss and (post one_sided rule) suppress real arbs. Join by slug cardinality (`_resolve_opp_ask`) or matcher-attached fields instead. Found by whole-branch review 2026-07-05.

24. [DATA] Never model both directions of one cross-venue pair as simultaneously arbitrage-priced in a test fixture — with sane two-sided books the four asks sum to ~2.04, so both directions beating ARB_THRESHOLD (2×0.96) is mathematically impossible; fixture each direction separately. Caught 2026-07-09 (T05 fixture, WNBA threshold tests).

25. [DATA] Never assume Kalshi game tickers carry a start time — WNBA game/spread/total tickers are date-only (`KXWNBASPREAD-26JUL09INDPHX-PHX7`), so `_kalshi_game_dt_utc` (%y%b%d%H%M) returns None for them. Use `_kalshi_game_date_only_utc` (midnight-ET anchor, `date_only` flag, 28h staleness window) and match games by ET-date equality, not the ±30-min window. MLB tickers DO carry HHMM.

26. [DATA] Polymarket US spread markets are one market with two team sides where the long side is the away team, NOT the title's cover team — "Phoenix wins by over 10.5" has phx on the short side. bestAsk in marketDataLite is the LONG side's ask, so store native long/short prices and a `yes_is_over` flag; translate to the cover frame only at match time. Verified live 2026-07-09.
