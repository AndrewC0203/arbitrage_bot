...
class KalshiOrderBook:
    ...
    not per ticker. (Mirrors ws_manager.KalshiOrderBook.)
    """
...
class KalshiPriceCache:
    ...
    def update_from_ws(self, ticker: str, msg: dict) -> None:
        ...
        Real feed carries yes_bid_dollars / yes_ask_dollars only — no_ask is
        derived as 1 - yes_bid. An explicit null / unusable price clears the
        side; only a MISSING key leaves it untouched. (Mirrors ws_manager.)
        """
        ...
        if "yes_ask_dollars" in msg or "yes_ask" in msg:
            entry["yes_ask"] = self._parse_price(msg.get("yes_ask_dollars", msg.get("yes_ask")))
            touched = True
...
def _handle_ws_message(data: dict) -> bool:
    ...
    (sequence gap on an orderbook sid stream).
