...
## [2026-07-02] Ghost filters — per-suppression ghost_log.jsonl (user override of "counters only") + filters gate would-be arbs, not every pair

Two deviations from the ghost-free-arbs design spec, both logged at implementation time. (1) The spec said per-suppression log events would recreate the 36k-noise problem and prescribed hourly counters only; the user explicitly asked for a log to study ghost patterns. Chose a SEPARATE `ghost_log.jsonl` (arb_log.jsonl stays tradeable-only) with a 60s per-(ticker, direction, reason) dedupe window — pattern data survives, tick-storm duplication doesn't. (2) F1/F2/F4 are evaluated per would-be arb (a direction whose fee-inclusive total beats ARB_THRESHOLD) rather than unconditionally at pair time: counters then mean "arbs suppressed" (comparable to the old ghost_rejected volume) instead of "pairs scanned" (thousands per 2s cycle, meaningless). Same suppression outcomes; only the accounting differs.
